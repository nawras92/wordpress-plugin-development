<?php
/*
 * Plugin Name: LWN Meta Tags 6
 * Description: Add custom meta tags from settings page
 * Author: Nawras Ali
 * Text Domain: meta-settings-6
 * */
...
function lwn_meta_tags_html_code($tags)
{
    if (isset($_GET['message']) && $_GET['message'] === 'ok') {
      ...
      '<div id="setting-error-settings_updated" class="notice notice-success settings-error is-dismissible">';
        $message_text .= '<p>';
        $message_text .= '<strong>';
        $message_text .= __('Settings Saved', 'meta-settings-6');
        $message_text .= '</strong>';
        $message_text .= '</p>';
    ...
    echo __('Meta Tags', 'meta-settings-6');
    echo '</h1>';
    echo '<hr />';
    echo '<h3 class="lwn-heading">';
    echo __('Add your tags here', 'meta-settings-6');
    echo '</h3>';
    // add your form here
    ...
    foreach ($tags as $index => $tag) {
        foreach ($tag as $key => $value) {
            if ($key === 'name') {
                $label = __('Meta Name', 'meta-settings-6');
            } elseif ($key === 'content') {
                $label = __('Meta Content', 'meta-settings-6');
            } else {
                $label = __('Unknown Meta Label', 'meta-settings-6');
            }
            echo '<div class="lwn-form-control" >';
            echo "<label class='lwn-label' for='lwn-meta-{$key}-{$index}'> ";
            echo $label;
            echo '</label> ';
            echo "<input name='lwn-meta-{$key}-{$index}' type='text' value='{$value}' />";
            echo '</div >';
        }
        echo '<hr />';
    }
...
function lwn_save_options_to_db()
{
    if (!current_user_can('manage_options')) {
        wp_die(
            __('You have no authorization to do this action.', 'meta-settings-6')
        );
    }
    ...
    if (isset($_POST)) {
        foreach ($options as $index => $meta_tag) {
            ...
            $updated_options[$index] = $posted_data;
        }
        // save to db
