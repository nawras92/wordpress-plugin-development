<?php
/*
 * Plugin Name: LWN Meta Tags 7
 * Description: Add custom meta tags from settings page
 * Author: Nawras Ali
 * Text Domain: meta-settings-7
 * */
...
/* function lwn_change_checkbox_value($name, $val) */
/* { */
/*     echo '<script type="text/javascript"'; */
/*     echo 'jQuery(function(){'; */
/*     echo "jQuery('input[name={$name}]').val({$val})"; */
/*     echo '})'; */
/*     echo '</script>'; */
/* } */
...
function lwn_meta_tags_html_code($tags)
{
    if (isset($_GET['message']) && $_GET['message'] === 'ok') {
      ...
      '<div id="setting-error-settings_updated" class="notice notice-success settings-error is-dismissible">';
        $message_text .= '<p>';
        $message_text .= '<strong>';
        $message_text .= __('Settings Saved', 'meta-settings-7');
        $message_text .= '</strong>';
        $message_text .= '</p>';
    ...
    echo __('Meta Tags', 'meta-settings-7');
    echo '</h1>';
    echo '<hr />';
    echo '<h3 class="lwn-heading">';
    echo __('Add your tags here', 'meta-settings-7');
    echo '</h3>';
    // add your form here
    ...
    foreach ($tags as $index => $tag) {
        foreach ($tag as $key => $value) {
            if ($key === 'name') {
                $label = __('Meta Name', 'meta-settings-7');
                $type = 'text';
                $checked = '';
            } elseif ($key === 'content') {
                $label = __('Meta Content', 'meta-settings-7');
                $type = 'text';
                $checked = '';
            } elseif ($key === 'show') {
                $label = __('Show Meta?', 'meta-settings-7');
                $type = 'checkbox';
                $checked = $value === 'yes' ? "checked='checked'" : '';
            } else {
                $label = __('Unknown Meta Label', 'meta-settings-7');
                $type = 'text';
                $checked = '';
            }
...
function lwn_save_options_to_db()
{
    if (!current_user_can('manage_options')) {
        wp_die(
            __('You have no authorization to do this action.', 'meta-settings-7')
        );
    }
        ...
        foreach ($options as $index => $meta_tag) {
            ...
            foreach ($meta_tag as $key => $value) {
                ...
                if (isset($_POST[$field_name])) {
                    if ($key === 'show') {
                        $new_value = $value === 'no' ? 'yes' : 'no';
                        // change value in dom directly
                        // a possible jquery solution
                        // it needs more work
                        /* lwn_change_checkbox_value($field_name, $new_value); */
