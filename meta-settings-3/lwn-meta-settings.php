<?php
/*
 * Plugin Name: LWN Meta Tags 3
 * Description: Add custom meta tags from settings page
 * Author: Nawras Ali
 * Text Domain: meta-settings-3
 * */
...
function lwn_meta_tags_html_code($tag_name, $tag_content)
{
    ...
    $html .= __('Your current meta name is: ', 'meta-settings-3');
    $html .= $tag_name;
    $html .= '</h4>';
    $html .= '<h4 class="lwn-heading">';
    $html .= __('Your current meta content is: ', 'meta-settings-3');
    $html .= $tag_content;
    $html .= '</h4>';
    $html .= '<hr />';
    $html .= '<h3 class="lwn-heading">';
    $html .= __('Add your tags here', 'meta-settings-3');
    $html .= '</h3>';
    // add your form here
...
function lwn_save_options_to_db()
{
    if (!current_user_can('manage_options')) {
        wp_die(
            __('You have no authorization to do this action.', 'meta-settings-3')
        );
    }
    ...
    if ($provided_name) {
        ...
        // add to db
        update_option('lwn_meta_tag_name', $clean_name);
    }
    if ($provided_content) {
