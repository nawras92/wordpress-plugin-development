<?php
/*
 * Plugin Name: LWN Meta Tags 5
 * Description: Add custom meta tags from settings page
 * Author: Nawras Ali
 * Text Domain: meta-settings-5
 * */
...
function lwn_meta_tags_html_code($tag_name, $tag_content)
{
    if (isset($_GET['message']) && $_GET['message'] === 'ok') {
      ...
      '<div id="setting-error-settings_updated" class="notice notice-success settings-error is-dismissible">';
        $message_text .= '<p>';
        $message_text .= '<strong>';
        $message_text .= __('Settings Saved', 'meta-settings-5');
        $message_text .= '</strong>';
        $message_text .= '</p>';
    ...
    $html .= __('Meta Tags', 'meta-settings-5');
    $html .= '</h1>';
    $html .= '<h5 class="lwn-heading">';
    $html .= __('Your current meta name is: ', 'meta-settings-5');
    $html .= $tag_name;
    $html .= '</h5>';
    $html .= '<h5 class="lwn-heading">';
    $html .= __('Your current meta content is: ', 'meta-settings-5');
    $html .= $tag_content;
    $html .= '</h5>';
    $html .= '<hr />';
    $html .= '<h3 class="lwn-heading">';
    $html .= __('Add your tags here', 'meta-settings-5');
    $html .= '</h3>';
    // add your form here
...
function lwn_save_options_to_db()
{
    if (!current_user_can('manage_options')) {
        wp_die(
            __('You have no authorization to do this action.', 'meta-settings-5')
        );
    }
