...
add_shortcode('lwn-recipes', 'lwn_recipe_shortcode');

function lwn_recipe_shortcode()
{
    ...
    if ($recipes_query->have_posts()) {
        ...
        while ($recipes_query->have_posts()) {
            ...
            if ($recipes_types) {
                ...
                $terms = __('Not classifed', 'lwn-recipe');
            }
            ...
            $html .= "<span class='recipe-meta-single'>" . __('Prep Time: ', 'lwn-recipe');
            $html .= esc_html(get_post_meta(get_the_ID(), 'lwn_recipe_prep_time', true));
            $html .= "</span>";
            $html .= "<span class='recipe-meta-single'>" . __('Cook Time: ', 'lwn-recipe');
            $html .= esc_html(get_post_meta(get_the_ID(), 'lwn_recipe_cook_time', true));
            $html .= "</span>";
            $html .= "<span class='recipe-meta-single'>" . __('Servings: ', 'lwn-recipe');
            $html .= esc_html(get_post_meta(get_the_ID(), 'lwn_recipe_servings', true));
            $html .= "</span>";
            $html .= "</div>";
            $html .= "<div class='recipe-terms'>";
            $html .= "<span class='recipe-term'>" . __('Types: ', 'lwn-recipe');
            $html .= esc_html($terms);
            $html .= "</span>";
            $html .= "</div>";
            $html .= "</div>";
        ...
        $html = __('There are no recipes', 'lwn-recipe');
    }
