<?php
/*
 * Plugin Name: LWN Meta Tags 9
 * Description: Add custom meta tags from settings page
 * Author: Nawras Ali
 * Text Domain: meta-settings-9
 * */
...
function lwn_meta_settings_menu()
{
    ...
    if (!empty($options_page)) {
        add_action('load-' . $options_page, 'lwn_meta_help_pages');
    }
}

// Help pages for our plugin
function lwn_meta_help_pages()
{
    $screen = get_current_screen();
    $screen->add_help_tab([
    'id' => 'lwn-meta-intro',
    'title' => 'Introduction',
    'content' => 'hiiiiiiiiiiiiiiiiiiiiii',
    'callback' => 'lwn_meta_help_introduction',
  ]);
    $screen->add_help_tab([
    'id' => 'lwn-meta-faq',
    'title' => 'FAQ',
    'content' => 'ur questions',
  ]);
    $screen->set_help_sidebar('<h3> sidebaaaaaaaaaaaaaar</h3>');
}

function lwn_meta_help_introduction()
{
    echo '<h3>Introduction</h3>';
    echo '<p>Amet adipisci possimus sunt saepe id. Repellendus autem eos dolorum minus voluptatibus Neque aliquid distinctio beatae ex deleniti? Sit natus</p>';
    echo __('hiiiiiiiiiiii', 'text domain');
}
...
function lwn_meta_tags_html_code($tags)
{
    if (isset($_GET['message']) && $_GET['message'] === 'ok') {
      ...
      '<div id="setting-error-settings_updated" class="notice notice-success settings-error is-dismissible">';
        $message_text .= '<p>';
        $message_text .= '<strong>';
        $message_text .= __('Settings Saved', 'meta-settings-9');
        $message_text .= '</strong>';
        $message_text .= '</p>';
    ...
    echo __('Meta Tags', 'meta-settings-9');
    echo '</h1>';
    echo '<hr />';
    echo '<h3 class="lwn-heading">';
    echo __('Add your tags here', 'meta-settings-9');
    echo '</h3>';
    // add your form here
    ...
    foreach ($tags as $index => $tag) {
        foreach ($tag as $key => $value) {
            if ($key === 'name') {
                $label = __('Meta Name', 'meta-settings-9');
                $type = 'text';
                $checked = '';
            } elseif ($key === 'content') {
                $label = __('Meta Content', 'meta-settings-9');
                $type = 'text';
                $checked = '';
            } elseif ($key === 'show') {
                $label = __('Show Meta?', 'meta-settings-9');
                $type = 'checkbox';
                $checked = $value === 'yes' ? "checked='checked'" : '';
            } else {
                $label = __('Unknown Meta Label', 'meta-settings-9');
                $type = 'text';
                $checked = '';
            }
...
function lwn_save_options_to_db()
{
    if (!current_user_can('manage_options')) {
        wp_die(
            __('You have no authorization to do this action.', 'meta-settings-9')
        );
    }
